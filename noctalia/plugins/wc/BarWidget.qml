...
Item {
  ...
  readonly property string displayText: {
    if (root.mainInstance.enabledTimezones.length === 0) return I18n.tr("world-clock.no-timezone");
    return `${root.mainInstance.currentCity} ${root.mainInstance.currentTime}`;
  }
  ...
  Rectangle {
    ...
    RowLayout {
      ...
      NText {
        id: cityText
        text: root.mainInstance.currentCity
        color: mouseArea.containsMouse ? Color.mOnHover : Color.mOnSurface
      ...
      NText {
        id: timeText
        text: root.mainInstance.currentTime
        color: mouseArea.containsMouse ? Color.mOnHover : Color.mOnSurface
        pointSize: root.barFontSize
    ...
    ColumnLayout {
      ...
      NText {
        text: root.mainInstance.currentTime.substring(0, 5)
        color: mouseArea.containsMouse ? Color.mOnHover : Color.mOnSurface
        pointSize: root.barFontSize * 0.65
        applyUiScale: false
        Layout.alignment: Qt.AlignHCenter
        visible: root.mainInstance.enabledTimezones.length > 0
      }
    }
  }
