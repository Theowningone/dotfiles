...

{
  ...
  # echo $closest > ~/.config/noctalia/templates/papirus-icons/final
  # echo $(dirname "$0") >> ~/.config/noctalia/templates/papirus-icons/final
  ...
  if [[ ! -d $HOME/.local/share/icons/$THEME ]]; then
    ...
    if [[ -d "/usr/share/icons/$THEME" ]]; then
      cp -r "/usr/share/icons/$THEME" "$HOME/.local/share/icons/"
    else
      echo "Error: $THEME Icons are not installed" 1>&2; exit 1
    fi
  fi

  # 6. Apply icons instantly
  # [[ -n "$closest" ]] && "$(dirname "$0")/papirus-folders" -C "$closest" || echo "Error: Failed to apply papirus-folders"
  [[ -n "$closest" ]] && papirus-folders -t "$THEME" -C "$closest" || echo "Error: Failed to apply papirus-folders"
}
